...
describe('notificationsSlice', () => {
    const initialState = {
        notifications: [],
        loading: false, // Ensure initial state has loading
    };

    test('Should return the initial state', () => {
        expect(notificationsSlice(undefined, { type: 'unknown' })).toEqual({
            notifications: [],
            loading: false, // Ensure loading is false in initial state
        });
    });
    ...
    describe('fetchNotifications async thunk', () => {
        test('Should handle fetchNotifications.pending', () => {
            ...
            expect(state).toEqual({
                ...initialState,
                loading: true, // loading should be true when the fetch is pending
            });
        });
        ...
        test('Should handle fetchNotifications.rejected', () => {
            ...
            expect(state).toEqual({
                ...initialState,
                loading: false, // loading should be false when fetch fails
            });
        });
